...
void preprocess(cv::Mat &imgOriginal, cv::Mat &imgGrayscale, cv::Mat &imgThresh) {
    imgGrayscale = extractValue(imgOriginal);                           //wartosc channela z oryginalnego obrazu

    cv::Mat imgMaxContrastGrayscale = maximizeContrast(imgGrayscale);       //maksymalizacja kontrastu

    cv::Mat imgBlurred;

    cv::GaussianBlur(imgMaxContrastGrayscale, imgBlurred, GAUSSIAN_SMOOTH_FILTER_SIZE, 0);          //blurr Gaussa
